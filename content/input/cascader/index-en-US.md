...
| remote | Whether to enable remote search. When enabled, local filtering is skipped on input change; only the `onSearch` callback is fired so that the consumer can update `treeData` asynchronously to render remote search results, mirroring Select's `remote` behavior | boolean | false | - |
