...
| form              | 外部传入的 formApi 实例，可通过 `Form.useForm()` 创建。用于在 Form 外部控制表单状态。                                                                                       | object                                        |            |
...
`Form.useForm()` 用于在 Form 组件**外部创建** formApi 实例，适合需要在 Form 渲染前就控制表单的场景。它返回一个数组 `[formApi, formState, formValues]`。
