...
#### 🎉 2.97.0 (2026-05-11)
- 【Feat】
    - Cascader supports remote search via the new `remote` prop. When enabled, search input no longer matches against local `treeData`; it only fires `onSearch`, leaving you to fetch and feed `treeData` asynchronously, mirroring the behavior of Select's `remote` [#1335](https://github.com/DouyinFE/semi-design/issues/1335) [#3232](https://github.com/DouyinFE/semi-design/pull/3232)
    - ConfigProvider adds `responsiveObserve` and `responsiveMap` props, allowing breakpoint queries to be configured globally and subscribed to via the `onBreakpoint` API on the ConfigConsumer context (aligned with Layout's responsive capability) [#1335](https://github.com/DouyinFE/semi-design/issues/1335) [#3253](https://github.com/DouyinFE/semi-design/pull/3253)
    - Form unifies validation API naming with the new `validator` prop, an alias for `validate` available at both form level and field level. `validator` takes precedence over `validate`. The relation between `validator` (used for programmatic submit-time validation) and `rules[].validator` (used for rule-level UI feedback) is also clarified — they are complementary and can coexist [#3233](https://github.com/DouyinFE/semi-design/pull/3233)
    - Image / ImagePreview add `initialZoom`, `maxZoom` and `minZoom` props, allowing the initial preview zoom ratio and zoom range to be customized [#1368](https://github.com/DouyinFE/semi-design/issues/1368) [#3255](https://github.com/DouyinFE/semi-design/pull/3255)
    - New SplitTagGroup component renders a group of Tags as a visually fused segment, automatically rounding the leading/trailing corners and dividing the middle ones [#3234](https://github.com/DouyinFE/semi-design/pull/3234)
    - Table adds the `headerStyle` prop for setting inline styles (background, border, etc.) on the header row directly, instead of going through className/CSS selectors [#2347](https://github.com/DouyinFE/semi-design/issues/2347) [#3246](https://github.com/DouyinFE/semi-design/pull/3246)
    - Tabs ships a draggable Tab demo built on top of `renderTabBar` + `@dnd-kit` [#2687](https://github.com/DouyinFE/semi-design/issues/2687) [#3238](https://github.com/DouyinFE/semi-design/pull/3238)
    - TagInput now wraps overflowed tags onto the next line via `flex-wrap`. While typing, the input area's width also follows the entered text so long inputs in narrow containers wrap to a new line instead of being squeezed into a single horizontally-scrollable line [#2715](https://github.com/DouyinFE/semi-design/issues/2715) [#3237](https://github.com/DouyinFE/semi-design/pull/3237)
    - TextArea adds the `resize` prop for native CSS resize behavior (`none` / `both` / `horizontal` / `vertical` / `block` / `inline`); it is automatically ignored when `autosize` is enabled. The `onResize` callback now also reports `{ height, width }` for native drag resize [#3243](https://github.com/DouyinFE/semi-design/pull/3243)
    - TimePicker `disabledTime` callback now receives a second argument `panelType` (`'left' | 'right'`) in range mode, so different disabled options can be returned for the start/end time panels. `disabledTime` only takes effect in range mode and is no longer invoked in single mode [#1443](https://github.com/DouyinFE/semi-design/issues/1443) [#3252](https://github.com/DouyinFE/semi-design/pull/3252)
    - Transfer adds the `virtualize` prop for the selected panel in `treeList` / list mode, addressing rendering perf issues when a large number of items are selected [#2586](https://github.com/DouyinFE/semi-design/issues/2586) [#3240](https://github.com/DouyinFE/semi-design/pull/3240)
    - Switch adds two independent knob background tokens `$switch-bgColor-knob_checked` / `$switch-bgColor-knob_unchecked`, allowing the knob color in checked/unchecked states to be customized independently [#2635](https://github.com/DouyinFE/semi-design/issues/2635) [#3236](https://github.com/DouyinFE/semi-design/pull/3236)
    - Upload supports image cropping when `accept` is image-typed: the new `crop` and `cropProps` props integrate with the Cropper component to crop selected images before upload [#3256](https://github.com/DouyinFE/semi-design/pull/3256)
- 【Fix】
    - Fix ConfigProvider with `responsiveObserve` enabled: subscribers to `onBreakpoint` used to receive the all-`false` constructor default on the immediate first callback. The first callback now synchronously evaluates `window.matchMedia` and delivers the real screen state [#3253](https://github.com/DouyinFE/semi-design/pull/3253)
    - Fix DatePicker panel width breaking under `density="compact"` when locale month names are long (e.g. Russian) [#2487](https://github.com/DouyinFE/semi-design/issues/2487) [#3244](https://github.com/DouyinFE/semi-design/pull/3244)
    - Fix Form Field `validate()` / `submit()` hanging forever when `rules[].message` is set to an empty string `''`. The legacy public-API semantics of `formApi.getError(field) === ''` is preserved (the empty string is kept as the field error value); `ErrorMessage` learns to recognize the "validation failed with empty message" state and skips rendering instead of falling back to `helpText` [#2340](https://github.com/DouyinFE/semi-design/issues/2340) [#3245](https://github.com/DouyinFE/semi-design/pull/3245)
    - Fix JsonViewer `autoWrap` not recomputing wrapping when the outer container width changes (now driven by `ResizeObserver` + `requestAnimationFrame` coalescing). Also chain `super.componentWillUnmount()` and `dispose()` the editor instance on unmount to prevent DOM listeners from leaking across mount cycles [#2837](https://github.com/DouyinFE/semi-design/issues/2837) [#3239](https://github.com/DouyinFE/semi-design/pull/3239)
    - Fix JsonViewer cursor landing position after replacing or pasting across a multi-line selection, with unified handling of LF / CRLF / CR line breaks; the cursor is now positioned at the end of the last line of the new text [#2908](https://github.com/DouyinFE/semi-design/issues/2908) [#3231](https://github.com/DouyinFE/semi-design/pull/3231)
    - Fix Navigation `footer` icon color token `$nav-footer-color-icon` not taking effect [#1841](https://github.com/DouyinFE/semi-design/issues/1841) [#3250](https://github.com/DouyinFE/semi-design/pull/3250)
    - Fix Select with `showClear` + `filter`: clicking the clear button and immediately blurring used to skip the blur notification, so Form validators never saw the cleared value. After clearing, the component now only refocuses without erroneously calling `open()`, keeping `!isOpen` so trigger blur reaches the notify path [#1453](https://github.com/DouyinFE/semi-design/issues/1453) [#3254](https://github.com/DouyinFE/semi-design/pull/3254)
    - Fix Spin SSR hydration mismatch on Next.js (`Prop 'id' did not match`) caused by a module-level gradient id counter that drifted between SSR and CSR. The first render uses a stable id, and the unique uuid is swapped in via `useEffect` on the client [#2114](https://github.com/DouyinFE/semi-design/issues/2114) [#3249](https://github.com/DouyinFE/semi-design/pull/3249)
    - Fix Table hitting `Maximum update depth exceeded` when columns are defined as JSX `<Table.Column>` children: derived column references kept thrashing, which made the data-cache recompute condition in `componentDidUpdate` perpetually true [#2592](https://github.com/DouyinFE/semi-design/issues/2592) [#3241](https://github.com/DouyinFE/semi-design/pull/3241)
    - Fix Table `sorter` columns swallowing the outer `onHeaderCell.onClick` due to internal stopPropagation [#3248](https://github.com/DouyinFE/semi-design/pull/3248)
    - Fix Tabs `collapsible` (scroll mode) flickering the collapse button when adding/removing tabs, caused by an `IntersectionObserver` ↔ render race in OverflowList [#3247](https://github.com/DouyinFE/semi-design/pull/3247)
    - Fix TimePicker single mode incorrectly invoking `disabledTime` with `panelType="left"` (a meaningless concept in single mode). `disabledTime` is now invoked only in range mode; single mode falls back to top-level `disabledHours` / `disabledMinutes` / `disabledSeconds` [#1443](https://github.com/DouyinFE/semi-design/issues/1443) [#3252](https://github.com/DouyinFE/semi-design/pull/3252)
    - Fix Toast running `content` component side effects (e.g. `useEffect`) twice on `Toast.close(id)`, caused by an intermediate render where `(isRemoved && !isAnimating)` rendered `<Toast />` as `null` and remounted it on the next paint. The same instance is now retained across the leave animation [#1483](https://github.com/DouyinFE/semi-design/issues/1483) [#3251](https://github.com/DouyinFE/semi-design/pull/3251)
