...
Set `type` to `custom` to obtain feedback in the form of multiple selections, and set the parameters of the multi-line input box through `renderContent`. When using custom feedback, you need to control whether the submit button is disabled or not. Users can set it through `okButtonProps`.
